...
export async function findUserByEmail(formData: FormData) {
  ...
  let message = intl.formatMessage({
    id: "actions.signUpForBackInStockSubscriptionAction.error",
  });
    ...
    if (response?.ok) {
      ...
      if (response.status === 202) {
        message = intl.formatMessage({
          id: "actions.signUpForBackInStockSubscriptionAction.success",
        });
      }
    } else {
      console.error(response?.status, response?.statusText);
      ...
      message = intl.formatMessage(
        { id: "actions.signUpForBackInStockSubscriptionAction.failed" },
        { status: response?.status },
      );
    }
  } catch (error) {
