  ...
  return (
    <div className="min-h-fullMinusNavbar relative z-40">
      ...
      <div className="sm:bg-dark grid max-w-[100dvw] gap-x-8 gap-y-4 bg-cover bg-fixed pt-20 sm:grid-cols-[1fr_3fr] sm:bg-[url('/images/backgrounds/MAV7957-Edit.jpg')] sm:px-8 sm:pt-16 sm:[grid-template-areas:'start_main'] lg:grid-cols-[1fr_48rem_1fr] lg:[grid-template-areas:'start_main_end']">
        <LogotypeIcon className="col-span-full sm:mb-8 h-8 justify-self-center fill-inherit drop-shadow transition duration-300 ease-in-out hover:scale-110 sm:h-10" />
        <main className="min-h-fullMinusNavbar sm:fill-light relative grid content-start sm:order-2 sm:mb-8 sm:max-w-[48rem] sm:[grid-area:main]">
          {children}
        </main>
        {!!customerAccessToken && (
          <Nav
            className="mb-8 [grid-area:start] sm:order-1 sm:mb-0 lg:min-w-96 lg:justify-self-end"
